...
class MainViewController : UIViewController {
  ...
  private func bind() {
    ...
    // FilterView 를 선택했을때 나오는 alertSheet를 선택했을 때 type
    let sortedType = alertActionTapped
      ...
      .startWith(.title) // 아무것도 건들지 않으면 테이블뷰의 시작값은 타이틀 별로 할거다.
    ...
    let alertForErrorMessage = blogError
      .map { message -> Alert in
        return (title : "앗!", message : "예상치 못한 오류가 발생했습니다. \(message)", actions : [.confirm], style : .alert)
      }
